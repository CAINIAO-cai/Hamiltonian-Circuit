...
class Graph:
    ...
    def getHamiltonian(self):
        ...
        """
        self.clearVisited()
        hamCircuit = Walk(self.totalVertices() + 1)
        hamCircuit.addVertex(0)
        if(self.tryVisiting(0, 1, hamCircuit)):
            hamCircuit.addVertex(0)
            return hamCircuit
        return None
    ...
    def tryVisiting(self, vertex, totalvisited, Hamiltonian):
        ...
        if(Hamiltonian.totalVertices() == self.totalV):
            if(self.edges[vertex][0] == 1):
                return True
        for i in range(self.totalV):
            self.visitedV[0] = True
            if (self.edges[vertex][i] == 1 and not self.visitedV[i]):
                self.visitedV[i] = True
